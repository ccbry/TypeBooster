...
VOID addHistory(score toadd, UINT ID) {
	...
	for (UINT i = 0; i < 5; i++) {
		...
		WriteProfile(hst, L"key_count", key_count, L"..\\etc\\history.ini");
		WCHAR error_count[5];
		...
		WCHAR time[10];
		StringCchPrintf(time, 10, L"%f", (DOUBLE)(pre[i].time));
...
score* readHistory(score* out, UINT ID) {
	...
	for (UINT i = 0; i < 5; i++) {
		...
		WCHAR tim[10];
		GetPrivateProfileString(hst, L"time", L"0.0", tim, 10, L"..\\etc\\history.ini");
		out[i].time = wcstof(tim, NULL) ;
